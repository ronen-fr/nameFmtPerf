...
//namespace ceph {
  ...
  public:
    ...
    void flush(std::byte* buf, size_t buf_len) override;
...
public:
    ...
    int test_extract(int n) override
    {
      return static_cast<uint8_t>(m_ss.view()[n]);
    }

    //using flush(std::byte* buf, size_t buf_len)
